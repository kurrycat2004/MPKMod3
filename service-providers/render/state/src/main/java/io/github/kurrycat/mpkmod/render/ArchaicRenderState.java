...
public class ArchaicRenderState implements RenderState {
    @AutoService(ServiceProvider.class)
    public static final class Provider extends RTServiceProvider<API, RenderState> {
        public Provider() {
            super(API::new, ArchaicRenderState::new, RenderState.class);
        }
    }
    ...
    protected static final class API extends RTServiceApi {
        ...
        private final MethodHandle getDisplayWidth = getter(
                MINECRAFT, int.class, "displayWidth", "field_71443_c"
        );
        private final MethodHandle getDisplayHeight = getter(
                MINECRAFT, int.class, "displayHeight", "field_71440_d"
        );
        private final MethodHandle newScaledResolution = constructor(
                SCALED_RESOLUTION,
                MethodType.methodType(void.class, MINECRAFT, int.class, int.class)
        );
        ...
        private final MethodHandle currentDisplayWidth = feed(getDisplayWidth, getMinecraft);
        private final MethodHandle currentDisplayHeight = feed(getDisplayHeight, getMinecraft);
        private final MethodHandle createScaledResolution = feed(
                newScaledResolution, getMinecraft, currentDisplayWidth, currentDisplayHeight
        );
    ...
    protected ArchaicRenderState(API api) {
        this.api = api;
    }
